...
export default function DriverDashboard() {
  ...
  useEffect(() => {
    if (isOnline && user && !socketRef.current) {
        ...
        ws.onopen = () => console.log("Connected to Driver Cloud");
          ...
          <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg">
            ...
            <div className="space-y-4">
              <div className="p-4 bg-gray-900 rounded border border-gray-600 font-mono text-sm">
                <p>📍 X: <span className="text-blue-400">{location.x.toFixed(2)}</span></p>
                <p>📍 Y: <span className="text-blue-400">{location.y.toFixed(2)}</span></p>
              </div>
              ...
              {!currentRide ? (
                ...
                >
                 ...
                 <div className="p-4 bg-blue-900/30 border border-blue-500/50 rounded-lg">
                    ...
                    <p className="text-sm text-gray-300">Destination: <span className="text-white font-bold">{currentRide.stations.name}</span></p>
                 </div>
              )}
            </div>
          </div>
